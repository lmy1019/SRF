...
for( i in 1:fold ){
  ...
  X.test = X[te.idx,  ];  #X.test <- scale(X.test)
  Z.test = Z[te.idx];  
  ...
  n.test = dim(X.test)[1]
  if(i == 1){
    output.mae = cbind( c(helper(pred, truth, std,weights)[1],
                    helper(naives$km.predictions,truth,rep(0, n.test),weights)[1],
                    helper(naives$cox.predictions,truth,rep(0, n.test),weights)[1],
                    helper(lu$lu.id.predictions,truth,rep(0, n.test),weights)[1],
                    helper(lu$lu.exp.predictions,truth,rep(0, n.test),weights)[1],
                    helper(wang$wang.id.predictions,truth,rep(0, n.test),weights)[1],
                    helper(wang$wang.exp.predictions,truth,rep(0, n.test),weights)[1]  
                    )
                  )
    
    output.rmse = cbind( c(helper(pred, truth, std,weights)[2],
                                     helper(naives$km.predictions,truth,rep(0, n.test),weights)[2],
                                     helper(naives$cox.predictions,truth,rep(0, n.test),weights)[2],
                                     helper(lu$lu.id.predictions,truth,rep(0, n.test),weights)[2],
                                     helper(lu$lu.exp.predictions,truth,rep(0, n.test),weights)[2],
                                     helper(wang$wang.id.predictions,truth,rep(0, n.test),weights)[2],
                                     helper(wang$wang.exp.predictions,truth,rep(0, n.test),weights)[2])
                      )
    
  }else{
    
    output.mae = cbind(output.mae,  c(helper(pred, truth, std,weights)[1],
                          helper(naives$km.predictions,truth,rep(0, n.test),weights)[1],
                          helper(naives$cox.predictions,truth,rep(0, n.test),weights)[1],
                          helper(lu$lu.id.predictions,truth,rep(0, n.test),weights)[1],
                          helper(lu$lu.exp.predictions,truth,rep(0, n.test),weights)[1],
    ...
    output.rmse = cbind(output.rmse,  c(helper(pred, truth, std,weights)[2],
                           helper(naives$km.predictions,truth,rep(0, n.test),weights)[2],
                           helper(naives$cox.predictions,truth,rep(0, n.test),weights)[2],
                           helper(lu$lu.id.predictions,truth,rep(0, n.test),weights)[2],
                           helper(lu$lu.exp.predictions,truth,rep(0, n.test),weights)[2],
...
row.names(output.rmse) <- c('srf',
                            ...
                            'w.id',
                            'w.exp')
colnames(output.mae)  <- c('fold.1','fold.2','fold.3','fold.4','fold.5','fold.6','fold.7','fold.8','fold.9','fold.10')
colnames(output.rmse)  <- c('fold.1','fold.2','fold.3','fold.4','fold.5','fold.6','fold.7','fold.8','fold.9','fold.10')

cat("L: ");cat(L);cat(", ")
cat("num_features: ");cat(num_features);cat(", \n")


boxplot(  data.frame(t(output.mae))   )
text(6, max(data.frame(t(output.mae)))*0.9, paste0("mae, L=",L),adj=c(0,0), col='red')
boxplot(  data.frame(t(output.rmse))   )
text(6, max(data.frame(t(output.rmse)) )*0.9, paste0("rmse, L=",L),adj=c(0,0), col='red')
