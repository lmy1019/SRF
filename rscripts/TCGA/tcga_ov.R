...
ov<-function(L = 5,num_features=10){
    ...
    #------------------------------------------------------------------------------------
    protein <- read.csv("/Users/lmy/Dropbox/Mingyang-HLi/projects/survival_random_forest/realdata/TCGA_OV_PROTEIN/protein.csv")
    ...
    clinic <- read.csv("/Users/lmy/Dropbox/Mingyang-HLi/projects/survival_random_forest/realdata/TCGA_OV_PROTEIN/clinic.csv")
    ...
    output = output/fold
    output = cbind( output, c(mean(pvalue.cutoff), median(pvalue.cutoff), c(0,0,0,0,0))  ) 
    row.names(output) <- c('custom.forest',
                           ...
                           'wang.id.estimator',
                           'wang.exp.estimator')
    colnames(output)  <- c('mae','rmse', 'pv.cutoff')
...
times = 20
result = list()
L.set=c(1,3,5,7,9,11)
for(L in L.set){
  #b[[ L ]]=NULL
  for(t in 1:times){
    cat("L: ");cat(L); cat("; t: ");cat(t); cat("\n")
    if(t == 1){result[[L]]=list()}
    result[[ L ]][[t]] = ov(L,10)
  }
  #result[[L]] = result[[L]]/times
...
for(L in L.set){
  ...
  for(t in 1:times){
      ...
      output.mae = cbind(output.mae, result[[L]][[t]][,1] )
      
      output.rmse = cbind( output.rmse,result[[L]][[t]][,2] )
      
      cutoff = rbind( cutoff, result[[L]][[t]][1:2,3]   )
      
  }
  
  label = c()
  for(ti in 1:20){ label = c(label, paste(c("fold",".",ti), collapse = '') ) }
  
  colnames(output.mae)  <- label
  colnames(output.rmse)  <- label
  colnames(cutoff)  <- c('pv.cutoff.mean', 'pv.cutoff.median')
  ...
  text(6, min(data.frame(t(output.mae)))+ (max(data.frame(t(output.mae)))-min(data.frame(t(output.mae))))*0.9, paste0("mae, L=",L),adj=c(0,0), col='red')
  boxplot(  data.frame(t(output.rmse))   )
  text(6, min(data.frame(t(output.rmse)))+ (max(data.frame(t(output.rmse)))-min(data.frame(t(output.rmse))))*0.9, paste0("rmse, L=",L),adj=c(0,0), col='red')
}
