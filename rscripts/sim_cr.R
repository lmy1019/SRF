...
sim_srf<- function(n = 500, p = 2, L = 1e17, hazardC = 0.2, SNR = 2, func='id',  times=5){
  ...
  X.time.ind.test =   matrix(runif(n*p,-1,1),n,p) 
  ...
  X.time.dep.test = -50*log((error.term+1)/2)+ rnorm(n,0,sigma)
  
  betaC = rep(0,p); betaC[1]=log(2);
  
  betaR = rep(0,p); betaR[1]=-log(3); betaR[p+1]=log(2)
  ...
  for( i in 1:times ){
    ...
    X.time.ind.tr = matrix(runif(n*p,-1,1),n,p)
    
    error.term = rnorm(n,0,sigma)
    
    X.time.dep.tr = -50*log((error.term+1)/2)+ rnorm(n,0,sigma)
