...
ov<-function(L = 5,num_features=10){
    ...
    for( i in 1:fold ){
      ...
      #training
      cat('Fold: '); cat(i); cat("...");if(i==fold){cat("\n")}
      ...
      for(j in 1:dim(data)[1]){
        #if(i%%1000==0){print(i)}
        cox.summary = summary(survfit(cox, data[j,]))
        z=Z[j]
      ...
      weights = c()#Ghat.old(surv_object, data.frame (X.tr),  Z.test.res, data.frame (X.test))
